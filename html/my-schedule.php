...
<!DOCTYPE html>
  <html>
    <head>
      <meta charset="UTF-8">
      <title>My Schedule | ND Air</title>
  ...
  <body>
    <nav>
      ...
      <ul>
          ...
          <li><a href="#" class="current">My Schedule</a></li>
          <li><a href="logout.php">Log Out</a></li>
      </ul>
    </nav>

  <main>
    <div class="title search-bar">My Schedule</div>
    <?php
    if ($_SESSION["title"] == 'p') {
	  ...
	  while ($row = oci_fetch_array($stmt, OCI_ASSOC)) {
		...
		echo '<div class="overlay" onclick="off('.$row['FLIGHT_ID'].')" style="display: none;" id="'.$row['FLIGHT_ID'].'">';
		  echo '<div class="info">';
			...
			// SHOW OTHER PILOTS
			/*$query = "select u.first_name, u.last_name, u.email from users u, flys where flys.flight_id = ".$row['FLIGHT_ID']." and u.pilot_id != ".$_SESSION['id']." and u.id = flys.pilot_id";
			$pilot_stmt = oci_parse($conn, $query);
			oci_execute($pilot_stmt);
			while ($pilot_row = oci_fetch_array($pilot_stmt, OCI_ASSOC)) {
				echo "<p>".$pilot_row['FIRST_NAME']."</p>";	
			}*/

			//SHOW OTHER FLIGHT ATTENDANTS
		  echo '</div>';
		echo '</div>';
      }
    echo "</table>";
